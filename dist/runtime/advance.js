import { START_NODE, END_NODE, } from '../graph/types.js';
...
export function advanceStructural(state, graph, ctx) {
    let mutated = false;
    ...
    for (let i = 0; i < MAX_ITERATIONS; i++) {
        ...
        if (leaf === END_NODE) {
            ...
            const subgraphNode = parentGraph.nodes.find((n) => n.kind === 'subgraph' && n.id === subgraphNodeId);
            if (!subgraphNode) {
