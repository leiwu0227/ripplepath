# demoflow — a dummy consumer CLI exercising ripplegraph end-to-end
...
it via `ripplegraph state` / `ripplegraph step`. This is the second assignment in
the ripplegraph project and its purpose is integration validation: prove that
...
the target project. After init, the user calls `ripplegraph state` / `step`
directly against `.demoflow/`. demoflow lives under `packages/demoflow/` in
this repo, depends on ripplegraph via `file:../..` for local development, and
